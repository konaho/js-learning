/**
* 　STEP7：画面遷移をつける（セレクタのみ表示→ボタンのみ表示→画像表示）
*    ・画面遷移用の関数を作る
*    ・不要な画面を非表示にするためのクラスを作る
*    ・画面遷移用の関数を呼び出すときに遷移先が指定できるようにする
*    ・画面遷移用の関数の中で、表示したい画面に従って各画面(section)にクラスをつけたり解除したりする
*    ・画面遷移用の関数の中で、各画面を表示するときに必要な処理があればそれを行う（もしくは各画面用の関数を作ってそれを呼び出す）
*    ・戻るボタンを作る
*    ・戻るボタンクリック用のイベントハンドラー関数を作る
*    ・戻るボタンにイベントハンドラーを設定する
